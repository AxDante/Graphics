...
WaveSystemRefraction::WaveSystemRefraction(int row, int col, float mass, float step)
{
	part_mass = mass;
	part_size = 0.35;
...
void WaveSystemRefraction::dragMotion() {
	return;
}


void WaveSystemRefraction::takeTimeStep()
{
	vector<Vector3f> f;
...
void WaveSystemRefraction::draw()
{
	...
	//if (count_wire == 0 || count_wire == 2 || count_wire == 3 || count_wire == 4) {
		for (int i = 0; i < state.size() / 2; i++) {
			...
			GLfloat particleColor[] = { 0.5f, 1.0f * (0.5 * pos[1] / 2 + 0.5), 0.5f, 1.0f };
...
int WaveSystemRefraction::index(int cur_row, int cur_col, int tot_row, int tot_col) {
...
void WaveSystemRefraction::takeStep(float stepSize)
{
