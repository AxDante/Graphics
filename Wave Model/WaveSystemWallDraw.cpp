...
WaveSystemWallDraw::WaveSystemWallDraw(int row, int col, float mass, float step)
{
	part_mass = mass;
	columns = col;
	rows = row;
...
void WaveSystemWallDraw::Initialize() {
	...
	center.push_back((int)rows * columns / 6 + columns / 2);
...
void WaveSystemWallDraw::dragMotion() {
	return;
}



void WaveSystemWallDraw::takeTimeStep()
{
	vector<Vector3f> f;
...
int WaveSystemWallDraw::index(int cur_row, int cur_col, int tot_row, int tot_col) {
...
void WaveSystemWallDraw::takeStep(float stepSize)
{
