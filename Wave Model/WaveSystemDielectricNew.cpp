...
WaveSystemDielectricNew::WaveSystemDielectricNew(int row, int col, float mass, float step)
{
	part_mass = mass;
	columns = col;
	rows = row;
...
void WaveSystemDielectricNew::Initialize() {
	...
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < columns; j++) {
			...
			particleRefractionIndex.push_back(1);
	...
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < columns; j++) {
			...
			if (i > 0.55*rows ) {
				...
				//}
				particleRefractionIndex[i * columns + j] = 2.0;
			}
...
void WaveSystemDielectricNew::dragMotion() {
	return;
}



void WaveSystemDielectricNew::takeTimeStep()
{
	vector<Vector3f> f;
	...
	for (int i = 0; i < nState.size() / 2; i++) {
		...
		float totY = 0;
		for (int source = 0; source < center.size(); source++) {
			totY += magnitudeStored[source][i] * sin(250  * sysCounter - phaseStored[source][i] * particleRefractionIndex[i]);
		}
...
void WaveSystemDielectricNew::draw()
{
	...
	//if (count_wire == 0 || count_wire == 2 || count_wire == 3 || count_wire == 4) {
		for (int i = 0; i < state.size() / 2; i++) {
			...
			if (particleType[i] == 0) {
				...
				GLfloat particleColor[] = { 0.5f, 1.0f * (0.5 * pos[1]  + 0.5), 0.5f + (particleRefractionIndex[i]-1.0)/10, 1.0f };
...
int WaveSystemDielectricNew::index(int cur_row, int cur_col, int tot_row, int tot_col) {
...
void WaveSystemDielectricNew::takeStep(float stepSize)
{
