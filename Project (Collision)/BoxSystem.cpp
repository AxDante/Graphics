...
BoxSystem::BoxSystem(float width, float length, float height, int n_particles, float mass)
{
	box_length = 4;
	box_thickness = 0.2;
	part_size = 0.25;
	...
	for (int k = 0; k < layers; k++) {
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < columns; j++) {
				...
				float rdm = 10*((double)rand() / (RAND_MAX));
				this->m_vVecState.push_back(Vector3f(r_init * i, r_init * j, r_init * k));
				this->m_vVecState.push_back(Vector3f(rdm, rdm +5, rdm-10));
			}
		}
	}
...
vector<Vector3f> BoxSystem::evalF(vector<Vector3f> state)
{
	...
	const float wall_k = 100.0; // wall spring constant
